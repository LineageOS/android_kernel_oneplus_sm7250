...
static struct tfa98xx *g_tfa98xx[4] = {0};
extern int aw882xx_register_success;
extern int aw882xx_i2c_probe(struct i2c_client *i2c,
	const struct i2c_device_id *id);
extern int aw882xx_i2c_remove(struct i2c_client *i2c);
int tfa98xx_register_success = 0;
EXPORT_SYMBOL(tfa98xx_register_success);
			     ...
			     const struct i2c_device_id *id)
{
	...
	if ((no_start == 0) && (no_reset == 0)) {
		...
		if (ret < 0) {
			...
			ret = aw882xx_i2c_probe(i2c, id);
			if (ret == 0)
				return ret; // aw smartPA register success
