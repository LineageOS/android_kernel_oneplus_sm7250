...
#ifdef CONFIG_SND_SOC_AWINIC_AW882XX
bool aw882xx_make_afe_callback(uint32_t token, uint32_t *payload,
				    uint32_t payload_size)
{
	pr_debug("%s:this_afe.aw_state = %d, payload_size = %d\n", __func__,
			atomic_read(&this_afe.aw_state), payload_size);
...
#ifdef CONFIG_SND_SOC_AWINIC_AW882XX
		if (aw882xx_make_afe_callback(data->token, data->payload,
					   data->payload_size))
			return 0;
#endif /*CONFIG_SND_SOC_AWINIC_AW882XX*/
